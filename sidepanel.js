...
const XAI_HOST = 'https://api.x.ai/v1';
...
async function getApiUsage() {
  ...
  return v || {
    ...
    grok: { inputTokens: 0, outputTokens: 0, lastUpdated: null }
  };
}
...
async function getKey(type = 'gemini') {
  const keyName = type === 'claude' ? 'claudeApiKey' : type === 'openai' ? 'openaiApiKey' : type === 'grok' ? 'grokApiKey' : 'geminiApiKey';
...
function bindSessionUI(root, s) {
  ...
  tplEl.value = s.tpl || '';
  srcEl.value = s.src || '';
  thinkingCheck.checked = s.thinkingEnabled || false;
  ...
  thinkingCheck.addEventListener('change', () => {
    s.thinkingEnabled = thinkingCheck.checked;
    s.updatedAt = now();
  });
  ...
  async function callGeminiText({ apiKey, token, model, text, systemPrompt, signal }) {
    ...
    if (apiKey) {
      ...
      throw new Error('APIキーまたはGoogleログインが必要です');
    }
    ...
    while (true) {
      ...
      if ((res.status === 429 || res.status === 503) && attempt < maxRetries) {
        ...
        const statusText = res.status === 429 ? '429 Quota Exceeded' : '503 Service Unavailable';
        console.log(`HTTP ${statusText}: Retrying in ${delay}ms...`);

        // Update UI status if possible (optional, but good for UX)
        // Note: 's' variable is from outer scope, so we can update it? 
        // Actually bindSessionUI is a closure, 's' is available but we need to update outEl.
        // We can't easily update UI text from here without passing a callback, 
        // but user will see '生成中…' which is fine.
      ...
      if (!res.ok) {
        ...
        catch (e) { throw new Error(`HTTP ${res.status} - ${raw.slice(0, 200)}`); }
      }
  ...
  async function callGrokText({ apiKey, model, text, systemPrompt, signal }) {
    const messages = [];
    if (systemPrompt) {
    ...
    const url = `${XAI_HOST}/chat/completions`;
    ...
    while (true) {
      ...
      const usage = data?.usage;
      if (usage) {
        await addApiUsage('grok', usage.prompt_tokens || 0, usage.completion_tokens || 0);
      }
      const choices = data?.choices || [];
      return choices.map(c => c.message?.content || '').join('');
    }
  ...
  async function callLocalLLMText({ url, model, text, systemPrompt, signal }) {
    const messages = [];
    if (systemPrompt) {
      messages.push({ role: 'system', content: systemPrompt });
    }
    ...
    if (!res.ok) {
      ...
      try { 
        const j = JSON.parse(raw); 
        const errMsg = (typeof j?.error === 'string') ? j.error : (j?.error?.message || j?.message);
        throw new Error(errMsg ? `HTTP ${res.status}: ${errMsg}` : `HTTP ${res.status}`); 
      }
      catch { throw new Error(`HTTP ${res.status}: エンドポイントURL（/v1忘れ等）かモデル名を確認してください`); }
    }

    const data = await res.json();
    const choices = data?.choices || [];
    let content = choices.map(c => c.message?.content || '').join('');
    
    // thinking mode が有効な場合、簡易的な抽出を試みる（タグがある場合）
    if (content.includes('<thought>')) {
      const parts = content.split(/<\/thought>/);
      if (parts.length > 1) {
        // 思考プロセスを分離して表示（あるいは隠すためのフラグとして利用可能）
        // 今回はシンプルに、思考部分を引用表示風にするなどの処理を検討
        content = parts.join('\n\n'); 
      }
    }
  ...
  async function onGenerate() {
    ...
    if (modelVal.startsWith('local-gemma-4')) {
      const gemmaMap = {
        'local-gemma-4-e2b': 'gemma4:e2b',
        'local-gemma-4-e4b': 'gemma4:e4b',
        'local-gemma-4-26b-a4b': 'gemma4:26b-a4b',
    ...
    if (apiType === 'gemini') {
      if (authPriority === 'google' && gToken) {
        // Priority: Google, and Token available -> Use Token
        finalToken = gToken;
      } else if (apiKeyRaw) {
        // Priority: API Key (default) OR Google (but token missing) -> Use API Key if available
        finalApiKey = apiKeyRaw;
      } else if (gToken) {
        // API Key missing -> Fallback to Token if available
        finalToken = gToken;
      } else {
        // Neither available
      }
    } else {
      // Claude/OpenAI/Grok just use API Key
      finalApiKey = apiKeyRaw;
    }
    ...
    if (!finalApiKey && !finalToken && !isLocal && !isHermesGrokWsl && !isHermesGptWsl && !isCodexAppServer) {
      ...
      outEl.textContent = isClaude
        ...
        : isOpenAI
          ...
          : isGrok
            ? 'Grok APIキーが未設定です。「設定」から保存してください。'
            : 'Gemini APIキー未設定、かつGoogleログインもされていません。';
    ...
    if (isLocal && !localUrl) {
      needKeyEl.style.display = 'block';
      outEl.textContent = 'ローカルLLMのエンドポイントURLが未設定です。「設定」から保存してください。';
      outEl.classList.remove('empty');
      return;
    }
...
chrome.storage.onChanged.addListener(async (changes, area) => {
  ...
  if ('geminiApiKey' in changes || 'claudeApiKey' in changes || 'openaiApiKey' in changes || 'grokApiKey' in changes) {
    const hasG = await hasKey('gemini');
    const hasC = await hasKey('claude');
    const hasO = await hasKey('openai');
    const hasGr = await hasKey('grok');
    if (needKeyEl) needKeyEl.style.display = (hasG || hasC || hasO || hasGr) ? 'none' : 'block';
  }
});
...
function updateAuthBtn() {
  ...
  if (gToken) {
    authBtn.textContent = 'G Sign out';
    authBtn.title = 'Googleアカウントからログアウト';
    authBtn.style.background = '#e8f0fe';
    authBtn.style.color = '#1967d2';
    authBtn.style.border = '1px solid #1967d2';
  } else {
    authBtn.textContent = 'G Sign in';
    authBtn.title = 'GoogleアカウントでログインしてGeminiを使用';
...
function updateKeyWarning(hasKey, hasToken) {
  needKeyEl.style.display = (hasKey || hasToken) ? 'none' : 'block';
}
...
authBtn.addEventListener('click', async () => {
  if (gToken) {
    ...
    await new Promise(resolve => {
      chrome.identity.removeCachedAuthToken({ token: gToken }, () => resolve());
    });
    gToken = null;
    toast('ログアウトしました');
  } else {
  ...
  const hasC = await hasKey('claude');
  const hasO = await hasKey('openai');
  updateKeyWarning(hasG || hasC || hasO, gToken);
});
